...
def calculate_accuracy(
    gold_labels: Dict[str, str], predictions: Dict[str, List[str]]
) -> float:
    score = 0.0
    ...
    if len(predictions) > 0:
        logging.error(
            "Found %d extra predictions, for example: %s",
            len(predictions),
...
def calculate_bleu(
    ...
    score = _compute_bleu(
        reference_corpus, prediction_corpus, max_order=max_order, smooth=smooth
    )[0]
        ...
        try:
            for row in reader:
                ...
                if instance_id in answers:
                    logging.error("Key %s repeated in %s", instance_id, filename)
                    sys.exit(EXIT_STATUS_ANSWERS_MALFORMED)
        ...
        try:
            for row in reader:
                ...
                if instance_id in predictions:
                    logging.error(
                        "Key %s repeated in file %s on line %d",
                        instance_id,
                        filename,
                        reader.line_num,
                    )
                ...
                if prediction == "":
                    logging.error(
                        "Key %s has empty labels for prediction in file %s on line %d",
                        instance_id,
                        filename,
                        reader.line_num,
                    )
    ...
    with open(filename, encoding="UTF-8", errors="replace") as f:
        ...
        try:
            for row in reader:
                ...
                if prediction_raw == "":
                    logging.warning(
                        "Key % s has empty prediction in file % s on line % d",
                        instance_id,
                        filename,
                        reader.line_num,
                    )
    ...
    if os.path.isdir(submit_dir) and os.path.isdir(truth_dir):
        ...
        for submission_file in submission_files:
            if submission_file not in [taskA_file, taskB_file, taskC_file]:
                logging.error(
                    "%s is not valid submission file name for any subtask!",
                    submission_file,
                )
                sys.exit(EXIT_STATUS_WRONG_FILE)
        if taskA_file in submission_files:
            gold_labels_file = os.path.join(truth_dir, "subtaskA_gold_answers.csv")
        ...
        if taskB_file in submission_files:
            gold_labels_file = os.path.join(truth_dir, "subtaskB_gold_answers.csv")
            gold_labels = read_gold_taskAB(gold_labels_file)
        ...
        if taskC_file in submission_files:
            gold_reference_file = os.path.join(truth_dir, "subtaskC_gold_answers.csv")
            ...
            bleu = calculate_bleu(references, predictions, max_order=4, smooth=False)
